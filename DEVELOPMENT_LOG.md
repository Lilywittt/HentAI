...
### [2026/01/20] 训练自动化套件深度优化

对训练与验证流程进行了工程化重构，实现了全自动参数加载与灵活的数据集接口。

- **全自动训练 WebUI (run_webui.sh)**:
  - **直接路径接口**: 支持通过命令行参数直接指定数据集路径，并实现智能缺省（自动载入最新语料）。
  - **静默配置载入**: 攻克了 Web 界面无法自动加载外部 YAML 的难题，通过“时间戳配置注入”和“缓存劫持”技术，实现了模型名称、输出路径、LoRA 参数的 100% 自动填充。
  - **动态索引生成**: 启动前自动扫描语料并重构 `dataset_info.json`，确保数据始终同步且无冗余垃圾文件。
- **存储与产物隔离**: 
  - 强制训练产物输出至数据盘 `/root/local-nvme`，实现项目代码与大数据量产物的完全解耦。
...
提升了清洗与转换脚本的复用性，并设计了三组核心数据生产模板。

#### 1. 脚本功能增强
- **`run_pipeline.py`**:
  - 新增 `--instruction` / `-ins` 和 `--schema` / `-sch` 参数。
  - 支持从命令行直接指定 `prompts/` 目录下的模板文件，实现清洗逻辑的快速切换。
- **`convert_to_lora.py`**:
  - **多路径合并**: `--input` 参数现在支持传入多个目录，自动汇总所有样本并生成统一的训练文件。
  - **移除映射表**: 删除了硬编码的 `MOOD_MAP`，改为直接输出 LLM 提取的原始情绪词汇。
  - **自定义输出**: 新增 `--output` 参数。

#### 2. 三大核心 Prompt 体系
在 `data_cleaning/prompts/` 下新增了三组模板，采用“Director (导演) - Actor (演员)”架构：
