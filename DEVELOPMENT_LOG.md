...
### [2026/01/17] 修复章节拆分逻辑 (Fix Chapter Splitting Logic)

修正了小说拆分脚本中关于“后篇”章节的归属错误，确保目录结构与原著层级一致。

- **问题描述**: 原脚本未识别“后篇”标志，导致“后篇”及其下属章节被错误合并到了“外篇”的最后一节中。
- **逻辑更新 (`split_novel.py`)**:
  - **正则增强**: 更新正则表达式，新增对 `--后篇--` 及 `后篇 第X章` 格式的匹配支持。
  - **独立卷管理**: 实现了 `11_Post_Chapters` 独立卷目录的创建逻辑，将其与“外篇”同级处理。
- **数据更新**: 重新运行了拆分脚本，更新了 `novel_data/split_data` 下的目录结构，现在包含独立的后篇章节目录。
